...
self.addEventListener('push', (event) => {
  ...
  let data: { type?: string; alerte_id?: number; num_chariot?: string; severite?: string; produit_ref?: string; produit_libelle?: string }
  try { data = event.data.json() } catch { return }

  const produitLabel = data.produit_libelle ?? data.produit_ref ?? 'Alerte'
